...
<div class="w-full max-w-sm p-6 bg-gray-200 rounded-lg  mt-10">
    <h2 class="text-2xl font-semibold text-center text-gray-800">Sign up</h2>
    <form action="{{ route('auth.register') }}" method="POST">
        @csrf
        <div class="mb-4">
            <label for="name" class="block text-sm font-medium text-gray-600">Name</label>
            <input type="name" id="name" name="name" required class="w-full px-4 py-2 mt-2 bg-white border rounded-lg focus:ring bg-">
            @error('name') <div class="text-red-500">{{ $message }}</div> @enderror
        </div>
        <div class="mb-4">
            
            <label for="email" class="block text-sm font-medium text-gray-600">Email</label>
            <input type="email" id="email" name="email" required class="w-full px-4 py-2 mt-2 bg-white border rounded-lg focus:ring">
        ...
        <div class="mb-4">
            ...
            <input type="contact" id="contact" name="contact" required class="w-full px-4 py-2 mt-2 bg-white border rounded-lg focus:ring">
